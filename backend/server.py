...
async def create_enquiry(input: EnquiryCreate):
    ...
    subject = f"New website enquiry: {input.service}"
    row = lambda label, value: (
        f'<tr><td style="padding:8px 16px 8px 0;font-family:Arial,sans-serif;font-size:13px;'
        f'color:#888;vertical-align:top;white-space:nowrap">{label}</td>'
        f'<td style="padding:8px 0;font-family:Arial,sans-serif;font-size:14px;color:#222">{value}</td></tr>'
    )
    html = (
        ...
        + row("Company", escape(input.company or "—"))
        + row("Service", escape(input.service))
        + row("Budget", escape(input.budget or "—"))
        + row("Received", escape(enquiry.created_at))
        + '</table>'
